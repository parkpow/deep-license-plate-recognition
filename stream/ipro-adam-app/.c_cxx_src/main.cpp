...
class PyGILStateLock {
 public:
  PyGILStateLock() { m_gstate = PyGILState_Ensure(); };
...
int main(int argc, char* argv[]) {
  ...
  Py_UNBLOCK_THREADS ADAM_Eventloop_Dispatch(s_systemEventloopId);
...
/* PyEval_InitThreads()：Python3.9以降非推奨のため、versionをチェックして呼び分ける */
#if (PY_MAJOR_VERSION == 3) && (PY_MINOR_VERSION < 9)
  /* Python3.9未満、これまで通りPyEval_InitThreads()を実行 */
  PyEval_InitThreads();
#else
  /* Python3.9以降、非推奨＝PyEval_InitThreads()を実行しない */
  ADAM_DEBUG_PRINT(ADAM_LV_INF, "PyEval_InitThreads() don't call: Python-Version[%d.%d.%d]\n",
...
#if 1
  do {
    if (factor == ADAM_STOP_FACTOR_APPLICATION) {
      break;
    }
...
//_save->thread_id);
    ...
    PyObject* pMemView =
        PyMemoryView_FromMemory(static_cast<char*>(pData->m_pData), pData->m_size, PyBUF_READ);
    if (pMemView == NULL) {
...
#endif
  //	ADAM_DEBUG_PRINT(ADAM_LV_DBG, "counter=%d tid=%ld\n", _save->gilstate_counter,
  //_save->thread_id); 	ADAM_DEBUG_PRINT(ADAM_LV_DBG, "Py_UNBLOCK_THREADS\n");
...
void sendHttpResponse(T_ADAM_REQUEST_ID reqId, PyObject* pArg) {
  ...
  ADAM_DEBUG_PRINT(ADAM_LV_DBG, "reqId=0x%" PRIxPTR ", headerType=%s, bodyType=%s\n", reqId,
                   Py_TYPE(pHttpHeader)->tp_name, Py_TYPE(pHttpBodyTmp)->tp_name);
