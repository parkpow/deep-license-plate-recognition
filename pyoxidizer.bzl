...
def make_exe(dist):
    ...
    python_config.run_module = "platerec_installer"
    ...
    # to our binary.
    exe.add_python_resources(exe.pip_install(["-r", "docker/requirements.txt"]))
    ...
    exe.add_python_resources(exe.read_package_root(
        path="docker",
        packages=["platerec_installer","stream_config"],
    ))
    # exe.add_python_resources(exe.read_package_root(CWD, ["src"]))
