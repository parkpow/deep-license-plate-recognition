...
def main():
    ...
    def verify_docker_install():
        try:
            output = subprocess.check_output("docker info --format '{{.ServerVersion}}'".split())
            return True
        except OSError:
            return False
    ...
    def install(image, auto_start_container, port, token, license_key, extra_args='', docker_version='docker', image_version='latest'):
    ...
    def get_image():
        images = subprocess.check_output("docker images --format '{{.Repository}}' platerecognizer/alpr*".split()).decode().split('\n')
    ...
    def get_token_input():
        ...
        if not token or not license_key or not verify_token(token, license_key):
            print("Invalid Token or License Key, get  license_key from https://app.platerecognizer.com/accounts/plan/")
    ...
    if action_choice == 1:
        hardwares = ('Intel CPU', 'Raspberry', 'GPU (Nvidia Only)', 'Jetson Nano', 'Quit')
        hardware = 1
        print('\n')
        ...
        while True:
            ...
            choice = int(input("What is the hardware of this machine [default=1] > ") or 1)    
        ...
        if hardware == 1:
            ...
            install(image, auto_start_container, port, token, license_key, extra_args='--runtume nvidia')

        elif hardware == 4:
            image = 'platerecognizer/alpr-jetson'
            install(image, auto_start_container, port, token, license_key, extra_args='--runtume nvidia', docker_version='nvidia-docker')

        exit(1)    
            
    elif action_choice == 2:
        version = str(input('Which version would you like to install? [default=latest]') or 'latest')    
        token, license_key = get_token_input() 
        image = get_image()
        if not image:
            print('PlateRecognizer SDK is not installed, Please select Install. Quiting!!')
            exit(1)
        ...
        auto_start_container = False    
        install(image, auto_start_container, 8080, token, license_key, extra_args=extra_args, docker_version=docker_version, image_version=version)
