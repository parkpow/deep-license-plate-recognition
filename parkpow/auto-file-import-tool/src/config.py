...
def parse_static_mapping(mapping_str: str) -> dict:
    ...
    try:
        return {
            ...
            for key, value in (item.split(":", 1) for item in mapping_str.split(","))
        }
    except ValueError as value_error:
        raise ValueError(
            f"Invalid static mapping format: {mapping_str}"
        ) from value_error
...
def parse_column_mapping(mapping_str: str) -> dict:
    ...
    try:
        for item in mapping_str.split(","):
            ...
            if not match:
                raise ValueError(
                    f"Invalid source column format: '{source}'. Expected 'ColumnX'."
                )
        ...
        raise ValueError(
            f"Invalid column mapping format: '{mapping_str}'. Details: {e}"
        ) from e
...
def load_config() -> dict[str, Any]:
    ...
    try:
        ...
        print(
            f"ERROR: Could not read the configuration file '{CONFIG_FILE}'. Please check its syntax or permissions."
        )
    ...
    expected_config_options = [
        ...
        ("CSV", "MAX_ROWS_PER_FILE", int, 45000, False),  # fallback is int, not str
    ]
            ...
            try:
                ...
                if is_required and isinstance(value, str) and not value.strip():
                    missing_parameters.append(
                        f"[{section}] {option} (REQUIRED, but is empty)"
                    )
                else:
                    app_config[option] = value
            except ValueError as e:
                missing_parameters.append(
                    f"[{section}] {option} (Invalid value or incorrect format: {e})"
                )
        else:
            if is_required:
                missing_parameters.append(
    ...
    if "BASE_API_URL" in app_config:
        ...
        missing_parameters.append(
            "[API] BASE_API_URL (REQUIRED, but not found or empty)"
        )
    ...
    if missing_parameters:
        print(
            f"ERROR: The configuration file '{CONFIG_FILE}' is missing or has incorrect values for the following parameters:"
        )
