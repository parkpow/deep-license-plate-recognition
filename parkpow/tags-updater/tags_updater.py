...
def list_vehicle_tags(api_token, base_url):
    ...
    while page > 0:
        ...
        res = requests.get(
            f"{base_url}/api/v1/vehicle-tags/",
            params={"page": page},
        ...
        if res.status_code == 200:
            ...
            raise Exception(f"An Error occurred listing vehicle tags: {res} {res.text}")
...
def process_vehicle_tag(
    vehicle_tag, data, vehicle_tags, parkpow_api_token, parkpow_base_url
):
    ...
    if vehicle_tag == NO_TAGS:
        ...
        removal = update_vehicle_tag(
            vehicle_id, vehicle_tag_id, False, parkpow_api_token, parkpow_base_url
        )

    # Adding new Tag
    new_vehicle_tag_id = vehicle_tags[new_vehicle_tag]
    addition = update_vehicle_tag(
        vehicle_id, new_vehicle_tag_id, True, parkpow_api_token, parkpow_base_url
    )

    if addition and removal:
        return [data["license_plate"][0], data["time"][0], vehicle_tag, new_vehicle_tag]


def process_alert(
    config_tags, raw_data, parkpow_api_token, parkpow_base_url, parkpow_tags
):
    """
    ...
    if "vehicle_tag" in data:
        for vehicle_tag in data["vehicle_tag"][0].split(","):
            ...
            if vehicle_tag in config_tags:
                return process_vehicle_tag(
                    vehicle_tag, data, parkpow_tags, parkpow_api_token, parkpow_base_url
                )
            else:
        ...
        if NO_TAGS in config_tags:
            return process_vehicle_tag(
                NO_TAGS, data, parkpow_tags, parkpow_api_token, parkpow_base_url
            )
        else:
        ...
        if content_type == "application/x-www-form-urlencoded":
            ...
            with open(log_file_path, "a", newline="") as csv_file:
                ...
                if not log_file_exists:
                    log_writer.writerow(["Plate", "Timestamp", "Old Tag", "New Tag"])
                    csv_file.flush()

                update = process_alert(
                    self.server.config,
                    raw_data,
...
class TagsUpdaterServer(HTTPServer):
    def __init__(
        self, server_address, config, settings, handler_class=AlertRequestHandler
    ):
        super().__init__(server_address, handler_class)
        self.config = config
        self.parkpow_api_token = settings["parkpow_api_token"]
        self.parkpow_base_url = settings.get(
            "parkpow_base_url", "https://app.parkpow.com"
        )
        self.vehicle_tags = list_vehicle_tags(
            self.parkpow_api_token, self.parkpow_base_url
        )
...
if __name__ == "__main__":
    ...
    server = TagsUpdaterServer(("", 8001), config, settings, AlertRequestHandler)
